...
const CLOUDINARY_FOLDER = 'DIYblinds/Inspiration/Expert Tips';
...
const uploadImage = async (
  ...
  try {
    ...
    await cloudinary.uploader.add_tag('Imported Expert Tip', [publicId]);
    if (context.length > 0) {
  ...
  for (const block of tip.contentBlocks) {
    if (block.type === 'image' && block.image) {
      ...
      if (!processedImages.has(imageSlug)) {
        tasks.push({
          filePath: `./.in/tips/${folder}/${imageSlug}`,
...
const saveProgress = (tips: Tip[], imageMappings: ImageMapping[], index: number): void => {
  if (index % 10 === 0) {
...
const run = async (): Promise<void> => {
  ...
  try {
    for (let index = 0; index < tips.length; index++) {
      ...
      const folder = sanitizeFolderName(tip.title);
      
      // Collect all distinct image upload tasks for this blog
      const uploadTasks = collectImageTasks(tip as any, folder);
      ...
      const tipImageMappings = updateBlogWithUploadedImages(tip as any, uploadResults);
      allImageMappings.push(...tipImageMappings);

      saveProgress(tips as any, allImageMappings, index);
    }
