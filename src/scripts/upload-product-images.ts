...
const rooms = ['study-room--rollerblinds']
const products = ['ref-sunscreen-roller-blinds']
const run = async () => {
    const files = [];
    const refs = []
    ...
    //fs.writeFileSync('./.in/refs-12.json', JSON.stringify(refs.filter(ref => products.includes(ref.product ?? '')), null, 2))
