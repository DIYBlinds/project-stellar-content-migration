...
import tipsJson from '../../.in/tips.json'
import fs from 'fs';
...
const run = (async () => {
  ...
  for (const tip of tips) {
    try {
      ...
      tip.title = await page.$eval('div.header__limit-title-width h1',  el => el.innerText);
      tip.heroImage = decodeBase64(await page.$eval('div.header--negative-feature-image img',  el => el.getAttribute('data-srcset')?.split(' ')[0] ?? ''));
      // delete shortDescription property from tour object if exists
      if (tip.shortDescription) {
        delete tip.shortDescription;
      } 
      ...
      for (const block of contentBlocks) {
        // Text columns
        if (await scrapeTextColumns(page, block, tip)) continue;
        // Richtext
        if (await scrapeRichtext(page, block, tip)) continue;
        // Content Tiles
        if (await scrapeContentTiles(page, block, tip)) continue;
        // Image
        if (await scrapeImage(page, block, tip)) continue;
        // Image Carousel
        if (await scrapeImageCarousel(page, block, tip)) continue;
        // gallery 
        if (await scrapeGallery(page, block, tip)) continue;
        // Video
        if (await scrapeVideo(page, block, tip)) continue;
        // Headline
        if (await scrapeHeadline(page, block, tip)) continue;
        // Fancy image panel
        if (await scrapeFancyImagePanel(page, block, tip)) continue;
        // Fancy image panel - flipped
        if (await scrapeFancyImagePanelFlipped(page, block, tip)) continue;
        // Text centered
        if (await scrapeTextCentered(page, block, tip)) continue;
        // Image left
        if (await scrapeImageLeft(page, block, tip)) continue;
      }
...
const scrapeTextColumns = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeRichtext = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeContentTiles = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeImage = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeImageCarousel = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeVideo = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeHeadline = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeFancyImagePanel = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeFancyImagePanelFlipped = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeGallery = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
...
const scrapeImageLeft = async (page: Page, block: ElementHandle<Element>, tip: Tip) => {
